...
def remove_floor_artifact(image: Image.Image) -> Image.Image:
    frame = image.copy().convert("RGBA")
    w, h = frame.size
    px = frame.load()
    start_y = int(h * 0.78)
    ...
    def is_floor_pixel(x: int, y: int) -> bool:
        r, g, b, a = px[x, y]
        if a == 0:
            return False
        brightness = max(r, g, b)
        saturation = brightness - min(r, g, b)
        return brightness >= 200 and saturation <= 18

    for y in range(start_y, h):
        for x in range(w):
            if (x, y) in seen or not is_floor_pixel(x, y):
                continue
            ...
            while queue:
                ...
                for ny in range(max(0, cy - 1), min(h, cy + 2)):
                    for nx in range(max(0, cx - 1), min(w, cx + 2)):
                        ...
                        if na >= 220 and max(nr, ng, nb) < 185:
                            touches_subject = True

                for nx, ny in ((cx - 1, cy), (cx + 1, cy), (cx, cy - 1), (cx, cy + 1)):
                    if 0 <= nx < w and 0 <= ny < h and (nx, ny) not in seen and is_floor_pixel(nx, ny):
                        seen.add((nx, ny))
                        queue.append((nx, ny))

            comp_w = max_x - min_x + 1
            comp_h = max_y - min_y + 1
            if not touches_subject and comp_h <= 14 and comp_w >= 10 and min_y >= start_y:
...
for i in range(getattr(im, "n_frames", 1)):
    ...
    clean = remove_floor_artifact(clean)
    bbox = clean.getbbox()
    if bbox is None:
